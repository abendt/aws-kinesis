...
public class Kinesis2Consumer extends ScheduledBatchPollingConsumer
    ...
    protected int poll() throws Exception {
        ...
        if (!getEndpoint().getConfiguration().getShardId().isEmpty()) {
            ...
            if (getEndpoint().getConfiguration().isAsyncClient()) {
                try {
                    response =
                            connection.getAsyncClient(getEndpoint()).describeStream(request).get();
                } catch (ExecutionException | InterruptedException e) {
                    throw new RuntimeException(e);
                }
            } else {
                response = connection.getClient(getEndpoint()).describeStream(request);
            }

            var shard =
                    response.streamDescription().shards().stream()
                            .filter(
                                    shardItem ->
                                            shardItem
                                                    .shardId()
                            ...
                            .findFirst()
                            .orElseThrow(
                                    () -> new IllegalStateException("The shard can't be found"));

            fetchAndPrepareRecordsForCamel(shard, connection, processedExchangeCount);

        } else {
            getShardList(connection).parallelStream()
                    .forEach(
                            shard -> {
                                fetchAndPrepareRecordsForCamel(
                                        shard, connection, processedExchangeCount);
                            });
        }
    ...
    private String getStreamARN() {
        if (streamARN == null) {
            ...
            LOG.debug("StreamARN is {}", streamARN);
        }
    ...
    private void fetchAndPrepareRecordsForCamel(
        ...
        if (getEndpoint().getConfiguration().isAsyncClient()) {
            try {
                result = kinesisConnection.getAsyncClient(getEndpoint()).getRecords(req).get();
            } catch (ExecutionException | InterruptedException e) {
                throw new RuntimeException(e);
            }
        } else {
    ...
    private String getShardIterator(final Shard shard, final KinesisConnection kinesisConnection)
        ...
        if (currentShardIterators.get(shardId) == null) {
            ...
            if (hasSequenceNumber()) {
                request.startingSequenceNumber(
                        getEndpoint().getConfiguration().getSequenceNumber());
            }
            ...
            LOG.debug(
                    "Obtained new ShardIterator {} for shard {} on stream {}",
    ...
    private void handleClosedShard(String shardId) {
        switch (getEndpoint().getConfiguration().getShardClosed()) {
            case ignore:
                LOG.warn(
                        "The shard with id={} on stream {} reached CLOSE status",
                        shardId,
                        getEndpoint().getConfiguration().getStreamName());
                break;
            case silent:
    ...
    private void resume(String shardId, GetShardIteratorRequest.Builder req) {
        ...
        if (adapter == null) {
            LOG.warn(
                    "There is a resume strategy setup, but no adapter configured or the type is incorrect");

            return;
        }

        adapter.configureGetShardIteratorRequest(
                req, getEndpoint().getConfiguration().getStreamName(), shardId);
    }
    ...
    protected Exchange createExchange(Shard shard, Record dataRecord) {
        LOG.debug(
                "Received Kinesis record with partition_key={} sequence_number={}",
        ...
        exchange.getIn()
                .setHeader(
                        Kinesis2Constants.APPROX_ARRIVAL_TIME,
    ...
    private List<Shard> getShardList(final KinesisConnection kinesisConnection) {
        ...
        if (getEndpoint().getConfiguration().isAsyncClient()) {
            ...
            shardList = kinesisConnection.getClient(getEndpoint()).listShards(request).shards();
        }
